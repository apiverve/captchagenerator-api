...
class CaptchageneratorValidationException implements Exception {
  final List<String> errors;

  CaptchageneratorValidationException(this.errors);

  @override
  String toString() => 'CaptchageneratorValidationException: ${errors.join("; ")}';
}
...
/// Captcha Generator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/captchagenerator?utm_source=dart&utm_medium=readme
///
/// Parameters:

class CaptchageneratorClient {
  final String apiKey;
  ...
  CaptchageneratorClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/captchagenerator',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [CaptchageneratorValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw CaptchageneratorValidationException(errors);
    }
  }

  /// Execute a request to the Captcha Generator API.
  ///
  /// Parameters are validated before sending the request.
  Future<CaptchageneratorResponse> execute() async {
    if (apiKey.isEmpty) {
      throw CaptchageneratorException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return CaptchageneratorResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw CaptchageneratorException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw CaptchageneratorException('Resource not found');
      } else {
        throw CaptchageneratorException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is CaptchageneratorException) rethrow;
      throw CaptchageneratorException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Captcha Generator API client.
class CaptchageneratorException implements Exception {
  final String message;

  CaptchageneratorException(this.message);

  @override
  String toString() => 'CaptchageneratorException: $message';
}
